...
from tests.base.data import test_docs_tenderer, test_docs_items_en
    ...
    def test_docs(self):
        ...
        with change_auth(self.app, ("Basic", ("brokerr", ""))):
            ...
            with open(TARGET_DIR + 'has-items-false-with-items-activate-error.http', 'w') as self.app.file_obj:
                response = self.app.patch_json(
                    '/frameworks/{}?acc_token={}'.format(self.framework_id, self.framework_token), {'data': {"status": "active"}},
                    status=422,
                )
        ...
        with change_auth(self.app, ("Basic", ("brokerr", ""))):
            ...
            with open(TARGET_DIR + 'has-items-false-without-items-agreement.http', 'w') as self.app.file_obj:
                response = self.app.get("/agreements/{}".format(self.agreement_id))
                self.assertEqual(response.status, "200 OK")
