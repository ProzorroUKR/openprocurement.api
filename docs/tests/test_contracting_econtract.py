...
test_tender_data_multi_buyers = deepcopy(test_tender_pq_multi_buyers_data)
...
class TenderResourceTest(BaseTenderWebTest, MockWebTestMixin):
    ...
    def test_docs(self):
        ...
        response = self.set_status(
            'active.tendering',
            extra={'auctionPeriod': {'startDate': (get_now() + timedelta(days=10)).isoformat()}}
        )
        ...
        # with open(TARGET_DIR + 'example_tender_contract.http', 'w') as self.app.file_obj:
        ...
        with open(TARGET_DIR + 'deprecated-contract-credentials.http', 'w') as self.app.file_obj:
            response = self.app.patch_json(
                f'/contracts/{contract_id}/credentials?acc_token={owner_token}'
            )
        self.assertEqual(response.status, '200 OK')
