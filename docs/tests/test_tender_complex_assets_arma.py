...
test_tender_data = deepcopy(test_docs_tender_complex_assets_arma)
...
TARGET_CSV_DIR = BASE_DIR + "complexAsset/csv/"
...
class TenderResourceTest(BaseTenderWebTest, MockWebTestMixin, TenderConfigCSVMixin):
    ...
    def test_docs(self):
        ...
        lots = test_lots[:1]
        lots[0]["id"] = uuid4().hex
        tender_data = {**test_tender_data, "lots": lots}
        ...
        with open(TARGET_DIR + "update-tender-after-enquiry.http", "w") as self.app.file_obj:
            response = self.app.patch_json(
                "/tenders/{}?acc_token={}".format(self.tender_id, owner_token),
        ...
        with open(TARGET_DIR + "qualified-bids-view.http", "w") as self.app.file_obj:
