...
.. csv-table::
   :file: complexAsset/csv/config.csv
   :header-rows: 1
...
.. csv-table::
   :file: complexAsset/csv/kind.csv
   :header-rows: 1
...
.. http:example:: complexAsset/http/tutorial/tender-post-attempt-json-data.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/blank-tender-view.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/tender-listing-no-auth.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/tender-add-relatedLot-to-item.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/notice-document-required.http
   :code:

Файл підпису повинен мати `documentType: notice` та `title: *.p7s`. Додамо такий документ:

.. http:example:: complexAsset/http/tutorial/add-notice-document.http
   :code:

Після додавання файлу підпису ми можемо активувати закупівлю:

.. http:example:: complexAsset/http/tutorial/tender-activating.http
   :code:

Подивимось, що показує список закупівель:

.. http:example:: complexAsset/http/tutorial/active-tender-listing-no-auth.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/patch-items-value-periods.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/tender-listing-after-patch.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/update-tender-after-enquiry.http
   :code:

Ось чому потрібно продовжити період подання пропозицій (`tenderPeriod`) на 7 днів.

.. http:example:: complexAsset/http/tutorial/update-tender-after-enquiry-with-update-periods.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/upload-tender-notice.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/tender-documents.http
   :code:

Один елемент масиву описує завантажений документ. Ми можемо завантажити більше документів:

.. http:example:: complexAsset/http/tutorial/upload-award-criteria.http
   :code:

І знову можна перевірити, що є два завантажених документа.

.. http:example:: complexAsset/http/tutorial/tender-documents-2.http
   :code:

Якщо сталась помилка, ми можемо ще раз завантажити документ поверх старої версії:

.. http:example:: complexAsset/http/tutorial/update-award-criteria.http
   :code:

І ми бачимо, що вона перекриває оригінальну версію:

.. http:example:: complexAsset/http/tutorial/tender-documents-3.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/ask-question.http
   :code:

Замовник може відповісти на них:

.. http:example:: complexAsset/http/tutorial/answer-question.http
   :code:

Можна отримати список запитань:

.. http:example:: complexAsset/http/tutorial/list-question.http
   :code:

або окрему відповідь:

.. http:example:: complexAsset/http/tutorial/get-answer.http
   :code:


Звернення можна задавати лише протягом періоду уточнень ``Tender.enquiryPeriod``

.. http:example:: complexAsset/http/tutorial/ask-question-after-enquiry-period.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/register-bidder.http
   :code:

Та додамо відповіді на вимоги критеріїв:

.. http:example:: complexAsset/http/tutorial/add-requirement-responses-to-bidder.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/activate-bidder-without-proposal.http
   :code:

Файл підпису має тип документу `documentType: proposal` та розширення `title: *.p7s`. Додамо файл підпису:

.. http:example:: complexAsset/http/tutorial/upload-bid-proposal.http
   :code:

Спробуємо підтвердити пропозицію:

.. http:example:: complexAsset/http/tutorial/activate-bidder.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/patch-pending-bid.http
   :code:

Якщо ми спробуємо підтвердити пропозицію, буде вимагатися новий підпис:

.. http:example:: complexAsset/http/tutorial/activate-bidder-without-sign.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/upload-bid-private-proposal.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/mark-bid-doc-private.http
   :code:

Можна перевірити завантажені документи:

.. http:example:: complexAsset/http/tutorial/bidder-documents.http
   :code:

.. _complexAsset_envelopes:
...
.. http:example:: complexAsset/http/tutorial/upload-bid-financial-document-proposal.http
   :code:

Отримати фінансові документи:

.. http:example:: complexAsset/http/tutorial/bidder-financial-documents.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/upload-bid-eligibility-document-proposal.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/upload-bid-qualification-document-proposal.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/bidder-view-financial-documents.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/bidder-after-changing-tender.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/bidder-activate-after-changing-tender.http
   :code:

Для того, щоб процедура відбулась, необхідно хоча б два учасника, тобто хоча б дві пропозиції повинні бути зареєстровані до початку аукціону:

.. http:example:: complexAsset/http/tutorial/register-2nd-bidder.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/register-3rd-bidder.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/qualifications-listing.http
   :code:

Підтвердіть перші дві пропозиції через кваліфікаційні об’єкти:

.. http:example:: complexAsset/http/tutorial/approve-qualification1.http
   :code:

.. http:example:: complexAsset/http/tutorial/approve-qualification2.http
   :code:

Можна відхилити пропозицію:

.. http:example:: complexAsset/http/tutorial/reject-qualification3.http
   :code:

Та перевірити, що вже кваліфіковані пропозиції переключені в стан `active`:

.. http:example:: complexAsset/http/tutorial/qualified-bids-view.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/rejected-bid-view.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/pre-qualification-sign-doc-is-required.http
   :code:

Файл підпису повинен мати `documentType: evaluationReports` та `title: *.p7s`. Додамо такий документ:

.. http:example:: complexAsset/http/tutorial/upload-evaluation-reports-doc.http
   :code:

Ще раз схвалимо кваліфікацію заявок:

.. http:example:: complexAsset/http/tutorial/pre-qualification-confirmation.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/auction-url.http
   :code:

Учасники можуть дізнатись свої URL-адреси для участі через свої пропозиції:

.. http:example:: complexAsset/http/tutorial/bidder-participation-url.http
   :code:

Дивіться на `Bid.participationUrl` у відповіді. Схожу, але іншу, URL-адресу можна отримати для інших учасників:

.. http:example:: complexAsset/http/tutorial/bidder2-participation-url.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/unsuccessful-qualified-award.http
   :code:

Спробуємо винести рішення по переможцю по аварду для `non-qualified` або `non-eligible` учасника:

.. http:example:: complexAsset/http/tutorial/activate-non-qualified-award.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/award-notice-document-required.http
   :code:

Така сама логіка при відхилені аварду:

.. http:example:: complexAsset/http/tutorial/award-unsuccessful-notice-document-required.http
   :code:
...
Е-Контрактінг недоступний для цього типу закупівель.

В режимі звичайного контрактінгу угода буде створена спочатку в системі закупівель.
...
.. http:example:: complexAsset/http/tutorial/prepare-cancellation.http
   :code:

Ви можете виправити тип на будь-який що вказаний вище.

.. http:example:: complexAsset/http/tutorial/update-cancellation-reasonType.http
   :code:
...
.. http:example:: complexAsset/http/tutorial/upload-cancellation-doc.http
   :code:

Змініть опис документа та інші властивості


.. http:example:: complexAsset/http/tutorial/patch-cancellation.http
   :code:

Завантажте нову версію документа


.. http:example:: complexAsset/http/tutorial/update-cancellation-doc.http
   :code:

Скасування
~~~~~~~~~~

Через відсутність періодів оскарження в цьому типі закупівлі скасування одразу будуть переходити в статус `active`.
...
.. http:example:: complexAsset/http/tutorial/cancellation-sign-doc-is-required.http
   :code:

Файл підпису повинен мати `documentType: cancellationReport` та `title: *.p7s`. Додамо такий документ:

.. http:example:: complexAsset/http/tutorial/upload-cancellation-report-doc.http
   :code:

Змінимо статус скасування з `draft` на `pending`:

.. http:example:: complexAsset/http/tutorial/pending-cancellation.http
   :code:

Скасування автоматично буде в статусі `active`, через відсутність періодів оскарження.

Закупівля теж змінить свій статус на `cancelled`, замовнику забороняються всі дії по закупівлі.