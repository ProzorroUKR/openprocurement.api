...
Пропозиція 1: Період розгляду + причина продовження (строкове поле)
------------------------------------------------------------------

Короткий опис:

Об'єкт `period` що створюється разом зі створенням аварду і змінює дату endDate при подовенні строків розгляду.

продовження строків розгляду відбувається за допомогою додавання причини продовження строковим полем.
...
продовжимо розгляд шляхом додавання причини продовження в поле `prolongationCause`:

.. sourcecode:: http

  PATCH /api/2.5/tenders/acc230d5bfa642958a52d3389b38f7b4/awards/62810a506e7f43a4831dd68565b516ce HTTP/1.0
  ...
  {
    "prolongationCause": "Обгрунтування продовження строків розгляду"
  }
...
при встановленні значення поля `prolongationCause` - система автоматично встановлює новий `period.endDate` як `period.startDate` + 20 робочих днів
...
Пропозиція 2: Період розгляду + причина продовження (об'єкт)
-----------------------------------------------------------
...
продовження строків розгляду відбувається за допомогою створення об'єкту `prolongation` з причиною продовження.
...
продовжимо розгляд шляхом додавання причини продовження в поле `prolongation`:

.. sourcecode:: http

  POST /api/2.5/tenders/acc230d5bfa642958a52d3389b38f7b4/awards/62810a506e7f43a4831dd68565b516ce/prolongation HTTP/1.0
  ...
  {
    "description": "Обгрунтування продовження строків розгляду",
    "dateCreated": "2024-01-04T17:30:00.000000+02:00",
  }
...
Пропозиція 3: Період розгляду + майлстоун
-----------------------------------------
