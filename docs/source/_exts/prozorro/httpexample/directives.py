...
class HTTPExample(CodeBlock):
    ...
    def run(self):
        ...
        # Final wrap with content wrapper
        container_node = nodes.container("", classes=["http-example"])

        # Add wrapper div that stretches to content size for global scroll
        wrapper_node = nodes.container("", classes=["http-example-wrapper"])
        wrapper_node.extend(result)

        container_node.append(wrapper_node)
