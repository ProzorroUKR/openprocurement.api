...
def test_update_status_invalid(app):
    ...
    assert response.json == {u'status': u'error', u'errors': [
        {u'description': [u'An active cancellation object is required'],
...
def test_cancel_plan_1_step(app):
    ...
    assert response.json == {"status": "error", "errors": [
        {"location": "body", "name": "cancellation",
         "description": {"reason": ["String value is too short."]}}]}
...
@pytest.mark.parametrize("value", [
    "aboveThresholdUA", "aboveThresholdUA.defense", "aboveThresholdEU", "esco",
    "competitiveDialogueUA", "competitiveDialogueEU", "closeFrameworkAgreementUA",
])
