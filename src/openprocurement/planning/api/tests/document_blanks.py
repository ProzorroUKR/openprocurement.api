...
def put_plan_document_remove_field(self):
    response = self.app.post_json(
        f"/plans/{self.plan_id}/documents?acc_token={self.plan_token}",
        {
            "data": {
                "title": "укр.doc",
                "url": self.generate_docservice_url(),
    ...
    response = self.app.put_json(
        f"/plans/{self.plan_id}/documents/{doc_id}?acc_token={self.plan_token}",
        {
            "data": {
                "title": "укр.doc",
                "url": self.generate_docservice_url(),
    ...
    document = [doc for doc in self.mongodb.plans.get(self.plan_id)["documents"] if doc["id"] == doc_id][-1]
