...
def put_framework_document_remove_field(self):
    response = self.app.post_json(
        f"/frameworks/{self.framework_id}/documents?acc_token={self.framework_token}",
        {
            "data": {
                "title": "укр.doc",
                "url": self.generate_docservice_url(),
    ...
    response = self.app.put_json(
        f"/frameworks/{self.framework_id}/documents/{doc_id}?acc_token={self.framework_token}",
        {
            "data": {
                "title": "укр.doc",
                "url": self.generate_docservice_url(),
    ...
    document = [doc for doc in self.mongodb.frameworks.get(self.framework_id)["documents"] if doc["id"] == doc_id][-1]
