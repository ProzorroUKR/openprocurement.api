...
def validate_award_document_tender_not_in_allowed_status_base(request, allowed_bot_statuses=("active.awarded",), **_):
    ...
    elif data := request.validated.get("data"):
...
def validate_document_operation_in_allowed_tender_statuses(allowed_statuses):
    ...
    def validate(request, **_):
        ...
        if request.authenticated_role == "auction":
            ...
            # Check if all documents are evaluation reports or acceptance reports
            if all(doc.get("documentType") in ("evaluationReports", "acceptanceReport") for doc in documents):
                # Allow operation in pre-qualification status for these document types
