...
class BaseShouldStartAfterMixing(baseclass):
...
class PreQualificationShouldStartAfterMixing(BaseShouldStartAfterMixing, baseclass):
    ...
    def get_lot_auction_should_start_after(self, tender, lot):
        if tender.get("status") not in ("active.pre-qualification.stand-still", "active.auction"):
            return
