...
from openprocurement.tender.core.procedure.serializers.config import (
    tender_config_has_multi_sourcing_migrate_value,
)
...
class Migration(PymongoCollectionMigration):
    description = "Migrate tender configs: hasMultiSourcing"
    ...
    def get_filter(self):
        return {"config.hasMultiSourcing": {"$exists": False}}

    def get_projection(self) -> dict:
        return {"config": 1, "procurementMethodType": 1}

    def update_document(self, doc, context=None):
        if doc["config"].get("hasMultiSourcing") is None:
            doc["config"]["hasMultiSourcing"] = tender_config_has_multi_sourcing_migrate_value(doc)
        return doc
