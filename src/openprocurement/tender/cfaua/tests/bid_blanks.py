...
def bids_invalidation_on_tender_change(self):
    ...
    response = self.app.post_json(
        '/tenders/{}/bids'.format(self.tender_id), {'data': initial_bids[0]}, status=422
    )
...
def get_tender_bidder_document(self):
    ...
    def all_documents_are_accessible_for_bid_owner(resource):
        ...
        for resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
            response = self.app.get('/tenders/{}/bids/{}/{}?acc_token={}'.format(self.tender_id, self.bid_id, resource,
                                                                                 self.bid_token))
            self.assertEqual(response.status, '200 OK')
    ...
    self.assertEqual(
        ...
        set([u'date', u'status', u'id', u'lotValues', u'tenderers', u'documents', u'eligibilityDocuments',
...
def create_tender_bidder_document(self):
    ...
    for doc_resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], "Can't upload document in current (active.pre-qualification) tender status")
    ...
    for doc_resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], "Can't upload document in current (active.pre-qualification.stand-still) tender status")
    ...
    for doc_resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], "Can't upload document in current (active.auction) tender status")
    ...
    for doc_resource in ['documents', 'eligibility_documents', 'qualification_documents']:
        response = self.app.post('/tenders/{}/bids/{}/{}?acc_token={}'.format(
            self.tender_id, self.bid2_id, doc_resource,  self.bid_token),
    ...
    response = self.app.post('/tenders/{}/bids/{}/financial_documents?acc_token={}'.format(
        self.tender_id, self.bid2_id,  self.bid_token),
        upload_files=[('file', 'name.doc', 'content')],
        status=201)
    ...
    for doc_resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], "Can't upload document in current (complete) tender status")
...
def put_tender_bidder_document(self):
    ...
    for doc_resource in ['documents', 'financial_documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.status, '403 Forbidden')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"], u"Can't upload document in current (active.pre-qualification) tender status")
    ...
    for doc_resource in ['documents', 'eligibility_documents', 'qualification_documents']:
        ...
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'][0]["description"],
            "Can't upload document in current (active.awarded) tender status")

    response = self.app.put('/tenders/{}/bids/{}/financial_documents/{}'.format(
...
def deleted_bid_do_not_locks_tender_in_state(self):
    ...
    response = self.app.delete(
        '/tenders/{}/bids/{}?acc_token={}'.format(self.tender_id, bid_id, bids_tokens[0])
    )
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
...
def download_tender_bidder_document(self):
    ...
    response = self.app.patch_json('/tenders/{}/auction/{}'.format(self.tender_id, self.initial_lots[0]['id']), {
        'data': {
            'lots': [{'auctionUrl': 'https://tender.auction.url'}],
            'bids': [
                {
                    'lotValues': [{'participationUrl': 'https://tender.auction.url/for_bid/{}'.format(i['id']),
    ...
    response = self.app.post_json('/tenders/{}/auction/{}'.format(self.tender_id, self.initial_lots[0]['id']), {'data': {'bids': auction_bids_data}})
    self.assertEqual(response.json['data']['status'], 'active.qualification')
    ...
    response = self.app.post_json(
        request_path,
        {
            'data': {
                ...
                'selfQualified': True,
                'tenderers': initial_bids[0]['tenderers'],
                'lotValues': [{"value": {"amount": 500, 'valueAddedTaxIncluded': False},
                               "relatedLot": self.initial_lots[0]['id']}]
            }
        },
        status=422
    ...
    response = self.app.post_json(
        request_path,
        {
            'data': {
                ...
                'selfQualified': True,
                'tenderers': initial_bids[0]['tenderers'],
                'lotValues': [{"value": {"amount": 500, 'currency': "USD"},
                               "relatedLot": self.initial_lots[0]['id']}]
            }
        },
        status=422
...
def get_tender_bidder_document_ds(self):
    ...
    self.assertEqual(len(response.json['data']), self.min_bids_number)
    self.assertEqual(set(response.json['data'][0].keys()),
                     set([u'date', u'status', u'id', u'lotValues', u'tenderers', u'documents',
                          u'eligibilityDocuments', u'qualificationDocuments', u'financialDocuments',
                          u'selfEligible', u'selfQualified']))
    self.assertEqual(set(response.json['data'][1].keys()), set([u'date', u'status', u'id', u'lotValues', u'tenderers',
                                                                u'selfEligible', u'selfQualified']))
    response = self.app.get('/tenders/{}/bids/{}'.format(self.tender_id, self.bid_id))
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(set(response.json['data'].keys()),
