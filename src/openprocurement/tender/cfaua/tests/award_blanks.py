...
def create_tender_award_invalid(self):
    ...
    response = self.app.post_json(
        request_path,
        {'data': {
            'suppliers': [{'identifier': 'invalid_value'}]
        }}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
...
def patch_tender_award(self):
    response = self.app.patch_json(
        '/tenders/{}/awards/some_id'.format(self.tender_id),
    ...
    response = self.app.patch_json(
        '/tenders/some_id/awards/some_id',
        {"data": {"status": "unsuccessful"}}, status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    ...
    self.assertEqual(
        response.json['errors'][0]["description"],
        "Can't update award in current (cancelled) status")
...
def create_tender_award_claim(self):
    ...
    self.app.patch_json(
        '/tenders/{}/awards/{}'.format(self.tender_id, unsuccessful_award_id),
        {'data': {'status': 'unsuccessful'}})
