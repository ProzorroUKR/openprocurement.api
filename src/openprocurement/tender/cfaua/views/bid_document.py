...
class TenderEUBidDocumentResource(TenderUaBidDocumentResource):
    ...
    def _upload_permission(self, tender_status):
        if tender_status not in self.upload_allowed_states:
            raise_operation_error(
                self.request, "Can't upload document in current ({}) tender status".format(tender_status)
            )
    ...
    def collection_get(self):
        ...
        if (
            self.request.validated["tender_status"] in self.view_forbidden_states
        ...
        if self.context.status in self.view_forbidden_bid_states and self.request.authenticated_role != "bid_owner":
            raise_operation_error(
                self.request, "Can't view bid documents in current ({}) bid status".format(self.context.status)
            )
        if self.request.params.get("all", ""):
            collection_data = [
                i.serialize("restricted_view") if self._doc_access_restricted(i) else i.serialize("view")
    ...
    def get(self):
        ...
        if (
            self.request.validated["bid"].status in self.view_forbidden_bid_states
            and self.request.authenticated_role != "bid_owner"
        ):
            raise_operation_error(
                self.request,
                "Can't view bid documents in current ({}) bid status".format(self.request.validated["bid"].status),
            )
