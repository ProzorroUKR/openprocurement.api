...
def create_tender_criteria_multi_profile(self):
    with mock.patch(
        "openprocurement.tender.pricequotation.procedure.models.tender.PQ_MULTI_PROFILE_FROM",
        get_now() + timedelta(days=1),
    ):
        with mock.patch(
            "openprocurement.tender.pricequotation.procedure.models.item.PQ_MULTI_PROFILE_FROM",
            get_now() + timedelta(days=1),
        ):
            with mock.patch(
                "openprocurement.tender.pricequotation.procedure.models.criterion.PQ_MULTI_PROFILE_FROM",
                get_now() + timedelta(days=1),
            ):
                ...
                self.assertEqual(
                    ...
                    [
                        {
                            "description": [{"relatesTo": ["Rogue field."], "relatedItem": ["Rogue field."]}],
                            "location": "body",
                            "name": "criteria",
                        }
                    ],
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            with mock.patch(
                ...
                self.assertEqual(
                    response.json["errors"],
                    [{"description": ["relatedItem should be one of items"], "location": "body", "name": "criteria"}],
                )
