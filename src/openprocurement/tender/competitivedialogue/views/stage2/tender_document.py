...
from openprocurement.tender.competitivedialogue.constants import STAGE_2_UA_TYPE, STAGE_2_EU_TYPE, STAGE2_STATUS
...
class CompetitiveDialogueStage2EUDocumentResource(TenderEUDocumentResource):
    def validate_update_tender(self):
        ...
        """
        if (
            self.request.authenticated_role != "auction"
            and self.request.validated["tender_status"] not in ["active.tendering", STAGE2_STATUS]
            or self.request.authenticated_role == "auction"
            and self.request.validated["tender_status"] not in ["active.auction", "active.qualification"]
        ):
            raise_operation_error(
                self.request,
                "Can't {} document in current ({}) tender status".format(
                    OPERATIONS.get(self.request.method), self.request.validated["tender_status"]
                ),
            )
        if (
            ...
            raise_operation_error(
                self.request, "tenderPeriod should be extended by {0.days} days".format(TENDERING_EXTRA_PERIOD)
            )
