...
def create_tender_lot(self):
    ...
    if self.initial_data["procurementMethodType"] not in ("belowThreshold",):
...
def get_tender_lot(self):
    ...
    self.assertEqual(
        ...
        {"id", "date", "title", "description", "minimalStep", "value", "status", "auctionPeriod"},
    )
...
def patch_tender_bid(self):
    ...
    self.assertEqual(response.status, "200 OK")
    self.assertEqual(response.content_type, "application/json")
    self.assertEqual(response.json["data"]["lotValues"][0]["value"]["amount"], 450)
    self.assertEqual(response.json["data"]["lotValues"][0]["date"], lot["date"])
