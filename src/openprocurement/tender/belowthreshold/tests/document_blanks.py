...
def tender_confidential_documents(self):
    ...
    response = self.app.get(
        f"/tenders/{self.tender_id}/documents/{doc_id}?acc_token={self.tender_token}&download={key}",
    )
    ...
    response = self.app.get(
        f"/tenders/{self.tender_id}/documents/{doc_id}?download={key}",
        status=403,
    )
    self.assertEqual(
        response.json,
...
def put_tender_document_remove_field(self):
    response = self.app.post_json(
        f"/tenders/{self.tender_id}/documents?acc_token={self.tender_token}",
        {
            "data": {
    ...
    document = [doc for doc in self.mongodb.tenders.get(self.tender_id)["documents"] if doc["id"] == doc_id][-1]
