...
def create_review_request(self):
    ...
    self.assertEqual(
        ...
        [{
            "location": "body",
            "name": "data",
            "description": "Review request can be created only in ('active.enquiries', 'active.awarded') tender statuses",
        }]
    )
    ...
    response = self.app.post_json(f"/tenders/{self.tender_id}/review_requests?acc_token={self.tender_token}")
...
def patch_review_request(self):
    ...
    response = self.app.get(f"/tenders/{self.tender_id}")
    self.assertEqual(response.status, "200 OK")
    if response.json["data"]["status"] != "active.awarded":
