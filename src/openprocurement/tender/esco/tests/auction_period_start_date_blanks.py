...
def tender_collection_put_auction_period_in_active_tendering(self):
    ...
    self.assertEqual(response.json, {'shouldStartAfter': '2022-01-05T00:00:00+02:00', 'startDate': '2022-01-25T14:06:17.298024+02:00'})


def tender_collection_put_auction_period_in_active_pre_qualification(self):
    self.app.authorization = ("Basic", ("administrator", ""))
    with freeze_time("2021-12-23"):
        start_date = (get_now() + timedelta(days=self.days_till_auction_starts)).isoformat()
    ...
    self.assertEqual(response.json,
                     {'shouldStartAfter': '2022-01-05T00:00:00+02:00', 'startDate': '2022-01-25T14:06:17.298024+02:00'})
...
def tender_lot_put_auction_period_in_active_tendering(self):
    ...
    self.assertEqual({'shouldStartAfter': '2022-01-05T00:00:00+02:00', 'startDate': '2022-01-25T14:06:17.298024+02:00'}, response.json)
...
def tender_lot_put_auction_period_in_active_pre_qualification(self):
    ...
    with freeze_time("2021-12-23"):
        start_date = (get_now() + timedelta(days=self.days_till_auction_starts)).isoformat()
        self.set_status("active.pre-qualification", {"lots": [
            {"auctionPeriod": {"startDate": start_date}}
            for i in self.initial_lots
        ]})
    response = self.app.put_json(
...
def tender_multe_lot_put_auction_period_in_active_pre_qualification(self):
    self.app.authorization = ("Basic", ("administrator", ""))
    lot_id1 = self.initial_lots[0]["id"]
    with freeze_time("2021-12-23"):
        start_date = (get_now() + timedelta(days=self.days_till_auction_starts)).isoformat()
