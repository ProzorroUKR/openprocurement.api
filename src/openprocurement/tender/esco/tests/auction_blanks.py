...
def post_tender_auction(self):
    ...
    self.assertEqual(
        tender["bids"][1]["value"]["yearlyPaymentsPercentage"],
    ...
    patch_data = {
        "bids": [
            ...
            },
            {
                ...
                "value": {"yearlyPaymentsPercentage": 0.85, "contractDuration": {"years": 10}},
            },
        ],
...
def auction_check_noticePublicationDate(self):
    ...
    response = self.app.patch_json(
        "/tenders/{}/auction".format(self.tender_id), {"data": {"noticePublicationDate": get_now().isoformat()}}
    )
    self.assertEqual(response.status, "200 OK")
    self.assertEqual(response.content_type, "application/json")
...
def post_tender_lot_auction(self):
    ...
    self.assertEqual(
        tender["bids"][1]["lotValues"][0]["value"]["yearlyPaymentsPercentage"],
...
def post_tender_auction_feature(self):
    ...
    patch_data = {
        "bids": [
            {
                "id": self.initial_bids[1]["id"],
                "value": {"yearlyPaymentsPercentage": 0.9, "contractDuration": {"years": 10, "days": 10}},
            }
        ]
    }
...
def post_tender_lot_auction_feature(self):
    ...
    patch_data["bids"].append(
        {"lotValues": [{"value": {"yearlyPaymentsPercentage": 0.9, "contractDuration": {"years": 10, "days": 10}}}]}
    )
