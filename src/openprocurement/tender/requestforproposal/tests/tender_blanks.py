            ...
            [
                {
                    "location": "body",
                    "name": "tenderPeriod",
                    "description": ["tenderPeriod must be at least 4 full business days long"],
                }
            ],
        )

    # all fine
    tender_end = calculate_tender_full_date(end_data, timedelta(days=4), tender=tender, working_days=True)
    response = self.app.patch_json(
        "/tenders/{}?acc_token={}".format(self.tender_id, self.tender_token),
        {
