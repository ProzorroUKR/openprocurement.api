...
from openprocurement.tender.belowthreshold.tests.utils import set_tender_lots, activate_contract
...
def patch_tender_currency(self):
    ...
    response = self.app.patch_json(
        ...
        {"data": {"value": {"currency": "GBP", "amount": self.initial_data["value"]["amount"]}, "items": items}},
        status=422,
    )
...
def proc_1lot_1bid_patch(self):
    ...
    lot_data.pop("id")
    response = self.app.post_json("/tenders/{}/lots?acc_token={}".format(tender_id, owner_token), {"data": lot_data})
    self.assertEqual(response.status, "201 Created")
    lot = response.json["data"]
    lot_id = lot["id"]
    # add relatedLot for item
...
def proc_1lot_3bid_1un(self):
    ...
    # sign contract
    self.app.authorization = ("Basic", ("broker", ""))
    activate_contract(self, tender_id, contract_id, owner_token, list(bids_data.items())[0][1])
    # check status
    self.app.authorization = ("Basic", ("broker", ""))
    response = self.app.get("/tenders/{}".format(tender_id))
    self.assertEqual(response.json["data"]["lots"][0]["status"], "complete")
...
def proc_2lot_2bid_1claim_1com_1win(self):
    ...
    response = self.app.post_json(
        "/tenders/{}/awards/{}/complaints?acc_token={}".format(tender_id, award_id, bid1_token),
        {"data": claim},
    )
    self.assertEqual(response.status, "201 Created")
    # cancel lot
