...
from openprocurement.tender.arma.tests.cancellation_blanks import (
        ...
        with change_auth(self.app, ("Basic", ("auction", ""))):
            ...
            for lot in self.initial_lots:
                patch_data = {
                    "lots": [
                        (
                            {"auctionUrl": f"http://auction.prozorro.gov.ua/{initial_lot['id']}"}
                            if initial_lot["id"] == lot["id"]
                            else {}
                        )
                        for initial_lot in self.initial_lots
                    ],
                    "bids": [
                        {
                            "lotValues": [
            ...
            patch_data = {
                "bids": [
                    {"lotValues": [{"value": {"amount": 1 + n}} for n, _ in enumerate(b.get("lotValues", []))]}
                    for b in auction_bids_data
                ]
            }
            for lot in self.initial_lots:
...
class TenderCancellationBidsAvailabilityTest(BaseTenderContentWebTest, TenderCancellationBidsAvailabilityUtils):
    initial_auth = ("Basic", ("broker", ""))
    initial_bids = test_tender_arma_bids * 2
