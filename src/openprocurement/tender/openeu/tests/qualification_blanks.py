...
def post_tender_qualifications(self):
    ...
    data = {"bidID": "1234" * 8, "status": "pending", "id": "12345678123456781234567812345678"}
    response = self.app.post_json(
        '/tenders/{}/qualifications/{}'.format(self.tender_id, data['id']),
    ...
    response = self.app.post_json(
        '/tenders/{}/qualifications/{}'.format(self.tender_id, qualifications[0]['id']),
        {"data": data}, status=405)
    self.assertEqual(response.status, '405 Method Not Allowed')
...
def patch_tender_qualifications(self):
    ...
    for status in ['pending', 'unsuccessful']:
        ...
        self.assertEqual(
            ...
            "Can't update qualification status")
...
def lot_get_tender_qualifications_collection(self):
    ...
    for bid in response.json['data']:
        response = self.app.get('/tenders/{}/bids/{}?acc_token={}'.format(
            self.tender_id, bid['id'], self.initial_bids_tokens[bid['id']]))
...
def create_qualification_document(self):
    ...
    response = self.app.get(
        '/tenders/{}/qualifications/{}/documents?all=true'.format(
            self.tender_id, self.qualifications[0]['id']))
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(doc_id, response.json["data"][0]["id"])
...
def create_tender_qualification_complaint_invalid(self):
    ...
    response = self.app.post_json(
        ...
        {'data': {
            'title': 'complaint title',
            'description': 'complaint description',
            'author': {'name': 'name', 'identifier': {'uri': 'invalid_value'}}}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
        {u'description': {
            u'contactPoint': [u'This field is required.'],
            u'identifier': {
                u'scheme': [u'This field is required.'], u'id': [u'This field is required.'],
...
def patch_tender_qualification_complaint(self):
    ...
    response = self.app.patch_json(
        '/tenders/{}/qualifications/{}/complaints/some_id'.format(
...
def create_tender_qualification_claim(self):
    response = self.app.post_json(
        '/tenders/{}/qualifications/{}/complaints?acc_token={}'.format(
            self.tender_id, self.unsuccessful_qualification_id,
...
def complaint_not_found(self):
    ...
    response = self.app.post(
        '/tenders/{}/qualifications/{}/complaints/some_id/documents'.format(
            self.tender_id, self.qualification_id),
        status=404,
        upload_files=[('file', 'name.doc', 'content')])
    self.assertEqual(response.status, '404 Not Found')
...
def create_tender_qualification_complaint_document(self):
    ...
    response = self.app.post(
        '/tenders/{}/qualifications/{}/complaints/{}/documents?acc_token={}'.format(
            self.tender_id, self.qualification_id,
            self.complaint_id, self.complaint_owner_token),
        upload_files=[('file', 'name.doc', 'content')], status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(
...
def put_tender_qualification_complaint_document(self):
    ...
    response = self.app.put(
        '/tenders/{}/qualifications/{}/complaints/{}/documents/{}?acc_token={}'.format(
            self.tender_id, self.qualification_id,
            self.complaint_id, doc_id, self.tender_token),
        upload_files=[('file', 'name.doc', 'content2')],
        status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.put(
        '/tenders/{}/qualifications/{}/complaints/{}/documents/{}?acc_token={}'.format(
            self.tender_id, self.qualification_id,
            self.complaint_id, doc_id, self.complaint_owner_token),
        upload_files=[('file', 'name.doc', 'content2')])
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(doc_id, response.json["data"]["id"])
...
def create_tender_2lot_qualification_complaint_document(self):
    response = self.app.post(
        '/tenders/{}/qualifications/{}/complaints/{}/documents?acc_token={}'.format(
            self.tender_id, self.qualification_id,
            self.complaint_id, self.tender_token),
        upload_files=[('file', 'name.doc', 'content')],
        status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
...
def switch_bid_status_unsuccessul_to_active(self):
    ...
    response = self.app.patch_json(
        '/tenders/{}/qualifications/{}?acc_token={}'.format(
            self.tender_id, new_qualification_id, self.tender_token),
        {"data": {'status': 'active', "qualified": True, "eligible": True}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.json['data']['status'], 'active')
