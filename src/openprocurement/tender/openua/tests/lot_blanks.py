...
def proc_1lot_1bid(self):
    ...
    response = self.set_status(
        'active.tendering',
        {"lots": [{"auctionPeriod": {"startDate": start_date.isoformat()}}]})
...
def proc_2lot_2bid_1lot_del(self):
    ...
    self.set_status(
        'active.tendering',
        {"lots": [{"auctionPeriod": {"startDate": start_date.isoformat()}} for i in lots]})
    # create bid
...
def proc_2lot_1bid_2com_1win(self):
    ...
    self.set_status(
        'active.auction',
        {"lots": [{"auctionPeriod": {"startDate": None}} for i in lots], 'status': 'active.tendering'})
    self.app.authorization = ('Basic', ('chronograph', ''))
...
def lots_features_delete(self):
    ...
    # add features
    self.app.patch_json('/tenders/{}?acc_token={}&opt_pretty=1'.format(tender['id'], owner_token), {'data': {
        "features": [
            {
                "code": "code_item",
                "featureOf": "item",
