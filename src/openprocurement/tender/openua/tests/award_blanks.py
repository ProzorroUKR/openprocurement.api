...
def create_tender_award_no_scale_invalid(self):
    self.app.authorization = ('Basic', ('token', ''))
    request_path = '/tenders/{}/awards'.format(self.tender_id)
    award_data = {'data': {'status': 'pending', 'bid_id': self.initial_bids[0]['id'], 'suppliers': [{
        key: value for key, value
        in test_organization.iteritems()
        if key != 'scale'
    }]}}
    response = self.app.post_json(request_path, award_data, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [{
...
def create_tender_award_with_scale_invalid(self):
    self.app.authorization = ('Basic', ('token', ''))
    request_path = '/tenders/{}/awards'.format(self.tender_id)
    award_data = {'data': {'status': 'pending',
...
def create_tender_award_no_scale(self):
    ...
    response = self.app.post_json(request_path, award_data)
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
