...
def validate_tender_period_extension_with_working_days(request):
    ...
    if request.validated["tender"].tenderPeriod.endDate < calculate_tender_business_date(
        get_now(), extra_period, tender, True
    ):
...
def validate_submit_claim_time(request):
    ...
    if get_now() > calculate_tender_business_date(tender.tenderPeriod.endDate, -claim_submit_time, tender, True):
