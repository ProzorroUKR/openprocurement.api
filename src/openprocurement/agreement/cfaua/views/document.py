...
from openprocurement.agreement.core.utils import context_unpack, save_agreement, apply_patch
from openprocurement.api.utils import json_view, APIResource, get_file, upload_file, update_file_content_type
...
class AgreementContractsResource(APIResource):
        ...
        if save_agreement(self.request):
            self.LOGGER.info(
                ...
                extra=context_unpack(
                    self.request, {"MESSAGE_ID": "agreement_document_create"}, {"document_id": document.id}
                ),
            )
    ...
    def put(self):
        ...
        if save_agreement(self.request):
            self.LOGGER.info(
                ...
                extra=context_unpack(self.request, {"MESSAGE_ID": "agreement_document_put"}),
            )
            return {"data": document.serialize("view")}
    ...
    def patch(self):
        ...
        if apply_patch(self.request, src=self.request.context.serialize()):
            ...
            self.LOGGER.info(
                "Updated agreement document {}".format(self.request.context.id),
                extra=context_unpack(self.request, {"MESSAGE_ID": "agreement_document_patch"}),
            )
