...
def not_found(self):
    ...
    self.assertEqual(response.json['errors'], [
        ...
        u'url', u'name': u'agreement_id'}
    ])

    response = self.app.post('/agreements/some_id/documents', status=404, upload_files=[
    ...
    response = self.app.post('/agreements/{}/documents?acc_token={}'.format(self.agreement_id, self.agreement_token), status=404, upload_files=[
    ...
    response = self.app.put('/agreements/some_id/documents/some_id', status=404, upload_files=[
    ...
    response = self.app.put('/agreements/{}/documents/some_id'.format(
        self.agreement_id), status=404, upload_files=[('file', 'name.doc', 'content2')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.get('/agreements/{}/documents/some_id'.format(
        self.agreement_id), status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
def put_contract_document(self):
    ...
    req = self.app.RequestClass.blank(self.app._remove_fragment('/agreements/{}/documents'.format(self.agreement_id)),
                                      environ)
    ...
    req = self.app.RequestClass.blank(
        self.app._remove_fragment('/agreements/{}/documents?acc_token={}'.format(self.agreement_id,
                                                                                 self.agreement_token)), environ)
    ...
    response = self.app.put('/agreements/{}/documents/{}?acc_token={}'.format(
        self.agreement_id, doc_id, self.agreement_token), upload_files=[('file', 'name  name.doc', 'content2')])
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.post('/agreements/{}/documents?acc_token={}'.format(
        self.agreement_id, self.agreement_token), upload_files=[('file', 'name.doc', 'content')])
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    doc_id = response.json["data"]['id']
    dateModified = response.json["data"]['dateModified']
    ...
    response = self.app.put(
        '/agreements/{}/documents/{}?acc_token={}'.format(self.agreement_id, doc_id, self.agreement_token), status=404,
        upload_files=[
            ('invalid_name', 'name.doc', 'content')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.put('/agreements/{}/documents/{}?acc_token={}'.format(
        self.agreement_id, doc_id, self.agreement_token), 'content3', content_type='application/msword')
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(doc_id, response.json["data"]["id"])
    if self.docservice:
    ...
    response = self.app.patch_json('/agreements/{}?acc_token={}'.format(self.agreement_id, self.agreement_token),
                                   {"data": {"status": "terminated"}}, content_type='application/json')
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.json['data']['status'], 'terminated')
    response = self.app.put('/agreements/{}/documents/{}?acc_token={}'.format(
        self.agreement_id, doc_id, self.agreement_token), 'contentX', content_type='application/msword', status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'],
    ...
    response = self.app.patch_json(
        '/agreements/{}/documents/{}?acc_token={}'.format(self.agreement_id, doc_id, self.agreement_token),
        {"data": {"documentType": None}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'], [
        {u'description': u"Can't update document in current (terminated)"
