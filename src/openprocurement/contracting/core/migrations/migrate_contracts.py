...
def create_contract(env, tender, tender_contract):
    if tender_contract.get("suppliers"):
        delete_buyers_attr(tender_contract["suppliers"])
    if tender.get("mode"):
    ...
    bid = next(
        (
            ...
            if i.get("id")
            == next((a.get("bid_id") for a in tender.get("awards", []) if a.get("id") == a.get("awardID")), None)
        ),
        tender,
    )

    tender_contract.pop("date", None)
...
def run(env, args):
    ...
    cursor = tenders_collection.find(
        {"contracts": {"$exists": True}, "procurementMethodType": {"$ne": "esco"}},
        {
            "contracts": 1,
            "buyers": 1,
            "bids": 1,
        ...
        for tender in cursor:
            ...
            contracting_contracts = {
                ...
                for i in contracts_collection.find(
                    {"_id": {"$in": contracts_ids}}, {"procuringEntity": 1, "buyerID": 1}
                )
            }
