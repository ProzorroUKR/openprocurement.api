...
from openprocurement.api.auth_async import get_login_middleware
from openprocurement.api.constants import ROUTE_PREFIX
from openprocurement.api.database_async import MongodbStore
from openprocurement.api.handlers.base import get_version, ping
from openprocurement.api.middlewares_async import (
...
def get_sub_app(global_config, **settings):
    ...
    wsgi_handler = WSGIHandler(
        pyramid_main(
            global_config,
            disable_route_prefix=True,  # we add it to the entire sup_app
            **settings,
        )
    )
...
def get_app(global_config, **settings):
    ...
    app = web.Application()
    app.add_subapp(ROUTE_PREFIX, sub_app)
    return app
