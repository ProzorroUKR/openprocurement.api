...
class IsoDurationTypeTest(BaseWebTest):
    def test_iso_duration_type(self):
        ...
        self.assertEqual(
            context.exception.message, ["ISO 8601 time designator 'T' missing. Unable to parse datetime string 'test'"]
        )
        with self.assertRaises(Exception) as context:
        ...
        self.assertEqual(context.exception.message, [u"Could not parse 123123. Should be ISO8601 Durations."])
