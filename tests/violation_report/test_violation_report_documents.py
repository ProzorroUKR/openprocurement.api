...
from tests.conftest import generate_test_doc_url
    ...
    async def test_patch_details(self, api):
        ...
        resp = await api.patch(
            ...
            json={
                "data": {
                    "title": "Підпис",
                    "documentType": DocumentTypes.violationReportSignature,
                }
            },
        )
        ...
        assert result["data"]["documentType"] == DocumentTypes.violationReportEvidence  # cannot change documentType
...
class TestPutDetailsDocument:
    async def test_put_details(self, api, sub_app):
        ...
        assert result["data"]["format"] == "sign/p7s"
