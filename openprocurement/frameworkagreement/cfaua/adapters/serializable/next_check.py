...
class SerializableTenderNextCheck(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.context.status == 'active.tendering' and self.context.tenderPeriod.endDate and \
            ...
            checks.append(self.context.tenderPeriod.endDate.astimezone(self.configurator.tz))
            ...
            if not any([
                ...
                checks.append(self.context.qualificationPeriod.endDate.astimezone(self.configurator.tz))
            ...
            if now < self.context.auctionPeriod.startDate:
                checks.append(self.context.auctionPeriod.startDate.astimezone(self.configurator.tz))
            elif now < calc_auction_end_time(self.context.numberOfBids, self.context.auctionPeriod.startDate).astimezone(self.configurator.tz):
                checks.append(calc_auction_end_time(self.context.numberOfBids, self.context.auctionPeriod.startDate).astimezone(self.configurator.tz))
        elif self.context.lots and self.context.status == 'active.auction':
            for lot in self.context.lots:
                if lot.status != 'active' or not lot.auctionPeriod or not lot.auctionPeriod.startDate or lot.auctionPeriod.endDate:
                    continue
                if now < lot.auctionPeriod.startDate:
                    checks.append(lot.auctionPeriod.startDate.astimezone(self.configurator.tz))
                elif now < calc_auction_end_time(lot.numberOfBids, lot.auctionPeriod.startDate).astimezone(self.configurator.tz):
                    checks.append(calc_auction_end_time(lot.numberOfBids, lot.auctionPeriod.startDate).astimezone(self.configurator.tz))
            ...
            standStillEnds = [
                a.complaintPeriod.endDate.astimezone(self.configurator.tz)
                for a in self.context.awards
