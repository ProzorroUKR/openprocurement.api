...
class TenderUaLotResource(TenderLotResource):
    ...
    @json_view(content_type="application/json", validators=(validate_patch_lot_data, validate_lot_operation_not_in_allowed_status), permission='edit_tender')
    def patch(self):
        """Update of lot
        """
        if not self.validate_update_tender():
    ...
    @json_view(permission='edit_tender', validators=(validate_lot_operation_not_in_allowed_status,))
    def delete(self):
        """Lot deleting
        """
        if not self.validate_update_tender():
