...
def one_lot_2bid_1unqualified_eu(self):
    self.app.authorization = ('Basic', ('broker', ''))
    # create tender
    response = self.app.post_json('/tenders', {"data": self.test_tender_data})
    tender_id = self.tender_id = response.json['data']['id']
...
def one_lot_3bid_1del_eu(self):
    ...
    # switch to active.pre-qualification
    self.time_shift('active.pre-qualification')
    self.check_chronograph() ### Тут після відпрацювання цього методу статус тендера стає unsuccessful, тому тест валиться, бо не можна патчити з таким статусом
...
def two_lot_2bid_0com_1can_eu(self):
    self.app.authorization = ('Basic', ('broker', ''))
    # create tender
    response = self.app.post_json('/tenders', {"data": self.test_tender_data})
    tender_id = self.tender_id = response.json['data']['id']
...
def one_lot_2bid_1unqualified_ua(self):
    self.app.authorization = ('Basic', ('broker', ''))
    # create tender
    response = self.app.post_json('/tenders', {"data": self.test_tender_data})
    tender_id = self.tender_id = response.json['data']['id']
...
def two_lot_2bid_0com_1can_ua(self):
    self.app.authorization = ('Basic', ('broker', ''))
    # create tender
    response = self.app.post_json('/tenders', {"data": self.test_tender_data})
    tender_id = self.tender_id = response.json['data']['id']
...
def two_lot_2bid_2com_2win_ua(self):
    ...
    self.assertEqual(response.status, '200 OK')
    # create bid
    bidder_data = deepcopy(self.test_bids_data[1]['tenderers'][0])
    bidder_data['identifier']['id'] = u"00037256"
    self.app.authorization = ('Basic', ('broker', ''))
    self.app.post_json('/tenders/{}/bids'.format(tender_id),
                      {'data': {'selfEligible': True,
