...
def create_tender_lot_invalid_eu(self):
...
def patch_tender_currency_eu(self):
    self.create_tender()
    lot = self.lots[0]
...
def tender_lot_guarantee_eu(self):
...
def tender_lot_guarantee_v2_eu(self):
    lots = deepcopy(self.initial_lots)
    lots[0]['guarantee'] = {"amount": 20, "currency": "GBP"}
...
def tender_features_invalid_eu(self):
...
def create_tender_bidder_invalid_eu(self):
    ...
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True, 'tenderers': self.test_bids_data[0]['tenderers'], 'lotValues': [{"value": {"amount": 5000000}, 'relatedLot': self.lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True, 'tenderers': self.test_bids_data[0]['tenderers'], 'lotValues': [{"value": {"amount": 500, 'valueAddedTaxIncluded': False}, 'relatedLot': self.lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True, 'tenderers': self.test_bids_data[0]['tenderers'], 'lotValues': [{"value": {"amount": 500, 'currency': "USD"}, 'relatedLot': self.lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True, 'tenderers': self.test_bids_data[0]['tenderers'], "value": {"amount": 500}, 'lotValues': [{"value": {"amount": 500}, 'relatedLot': self.lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
def patch_tender_bidder_eu(self):
...
def create_tender_with_features_bidder_invalid_eu(self):
    tenderers = deepcopy(self.test_bids_data[0]["tenderers"])
    tenderers[0]['identifier']['id'] = self.initial_data['shortlistedFirms'][0]['identifier']['id']
    tenderers[0]['identifier']['scheme'] = self.initial_data['shortlistedFirms'][0]['identifier']['scheme']
...
def create_tender_with_features_bidder_eu(self):
    tenderers = deepcopy(self.test_bids_data[0]["tenderers"])
    tenderers[0]['identifier']['id'] = self.initial_data['shortlistedFirms'][0]['identifier']['id']
    tenderers[0]['identifier']['scheme'] = self.initial_data['shortlistedFirms'][0]['identifier']['scheme']
...
def one_lot_0bid_eu(self):
    ...
    response = self.app.post_json('/tenders/{}/lots?acc_token={}'.format(self.tender_id, self.tender_token),
                                  {'data': self.test_lots_data[0]}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'], [{
...
def two_lot_2bid_1lot_del_eu(self):
    # create tender 2 lot
...
def two_lot_2bid_2com_2win_eu(self):
    """ Create tender with 2 lots and 2 bids """
    self.create_tender(initial_lots=self.test_lots_data*2)
    tenderers = self.create_tenderers(2)
    # create bid
...
def create_tender_lot_invalid_ua(self):
    """ Try create invalid lot """
...
def patch_tender_currency_ua(self):
    self.create_tender()
    lot = self.lots[0]
    self.assertEqual(lot['value']['currency'], "UAH")
...
def tender_lot_guarantee_ua(self):
    lots = deepcopy(self.initial_lots)
    lots[0]['guarantee'] = {"amount": 20, "currency": "GBP"}
    self.create_tender(initial_lots=lots)
    lot = self.lots[0]
...
def tender_lot_guarantee_v2_ua(self):
    lots = deepcopy(self.initial_lots)
    lots[0]['guarantee'] = {"amount": 20, "currency": "GBP"}
    lots[1]['guarantee'] = {"amount": 40, "currency": "GBP"}
    self.create_tender(initial_lots=lots)
...
def tender_features_invalid_ua(self):
    request_path = '/tenders/{}?acc_token={}'.format(self.tender_id, self.tender_token)
    data = self.test_tender_data.copy()
    item = data['items'][0].copy()
    item['id'] = "1"
...
def create_tender_bidder_invalid_ua(self):
    request_path = '/tenders/{}/bids'.format(self.tender_id)
...
def create_tender_with_features_bidder_invalid_ua(self):
    request_path = '/tenders/{}/bids'.format(self.tender_id)
    tenderers = self.create_tenderers()
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True,
...
def create_tender_with_features_bidder_ua(self):
    request_path = '/tenders/{}/bids'.format(self.tender_id)
    tenderers = self.create_tenderers()
    response = self.app.post_json(request_path, {'data': {'selfEligible': True, 'selfQualified': True,
                                                          'tenderers': tenderers[0],
...
def two_lot_2bid_1lot_del_ua(self):
    self.create_tender(initial_lots=self.test_lots_data*2)
    tenderers = self.create_tenderers(2)
    self.app.authorization = ('Basic', ('broker', ''))
    # switch to active.tendering
