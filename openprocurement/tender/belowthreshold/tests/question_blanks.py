...
def patch_tender_question(self):
    ...
    self.set_status(self.test_forbidden_question_actions_status)
    ...
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"], "Can't update question in current ({}) tender status".format(self.test_forbidden_question_actions_status))
