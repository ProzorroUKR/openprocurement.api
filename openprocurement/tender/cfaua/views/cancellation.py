...
@optendersresource(name='closeFrameworkAgreementUA:Tender Cancellations',
...
class TenderCancellationResource(BaseResource):
    ...
    def cancel_tender(self):
        ...
        if tender.status in ['active.awarded']:
            [setattr(i, 'status', 'cancelled') for i in tender.lots if i.status in ['pending', 'active']]
            [setattr(i, 'status', 'cancelled') for i in tender.agreements if i.status in ['pending', 'active']]
            [setattr(i, 'status', 'cancelled') for i in tender.qualifications if i.status in ['pending', 'active']]
        ...
        if tender.status == 'active.awarded' and tender.agreements:
            for agreement in tender.agreements:
                for item in agreement.items:
                    if item.relatedLot in cancelled_lots:
