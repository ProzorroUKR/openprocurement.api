...
from openprocurement.api.utils import update_logging_context, error_handler, get_now  # XXX tender context
...
def validate_lot_operation_with_awards(request):
    tender = request.validated['tender']
    if tender.awards:
        request.errors.add('body', 'data', 'Can\'t {} lot when you have awards'.format(OPERATIONS.get(request.method), tender.status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_award_operation_not_in_active_status(request):
    tender = request.validated['tender']
    if tender.status != 'active':
        request.errors.add('body', 'data', 'Can\'t {} award in current ({}) tender status'.format('create' if request.method == 'POST' else 'update', tender.status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_create_new_award(request):
    ...
    if tender.awards and tender.awards[-1].status in ['pending', 'active']:
        request.errors.add('body', 'data', 'Can\'t create new award while any ({}) award exists'.format(tender.awards[-1].status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_lot_cancellation(request):
    ...
    if tender.get('lots') and tender.get('cancellations') and [cancellation for cancellation in tender.get('cancellations', []) if cancellation.get('relatedLot') == award.lotID]:
        request.errors.add('body', 'data', 'Can\'t {} award while cancellation for corresponding lot exists'.format(OPERATIONS.get(request.method)))
        request.errors.status = 403
        raise error_handler(request.errors)
    ...
    if tender.awards:
        if tender.lots:  # If tender with lots
            if award.lotID in [aw.lotID for aw in tender.awards if aw.status in ['pending', 'active']]:
                request.errors.add('body', 'data', 'Can\'t create new award on lot while any ({}) award exists'.format(tender.awards[-1].status))
                request.errors.status = 403
                raise error_handler(request.errors)
        else:
...
def validate_award_document_add_not_in_pending(request):
    if request.validated['award'].status != 'pending':
        request.errors.add('body', 'data', 'Can\'t add document in current ({}) award status'.format(request.validated['award'].status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_award_complaint_operation_not_in_active(request):
    tender = request.validated['tender']
    if tender.status != 'active':
        request.errors.add('body', 'data', 'Can\'t {} complaint in current ({}) tender status'.format(OPERATIONS.get(request.method), tender.status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_contract_operation_not_in_active(request):
    if request.validated['tender_status'] not in ['active']:
        request.errors.add('body', 'data', 'Can\'t {} contract in current ({}) tender status'.format(OPERATIONS.get(request.method), request.validated['tender_status']))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_contract_update_in_cancelled(request):
    if request.context.status == 'cancelled':
        request.errors.add('body', 'data', 'Can\'t update contract in current ({}) status'.format(request.context.status))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_contract_with_cancellations_and_contract_signing(request):
    ...
    if request.context.status != 'active' and 'status' in data and data['status'] == 'active':
        ...
        if tender.get('lots') and tender.get('cancellations') and [cancellation for cancellation in tender.get('cancellations') if cancellation.get('relatedLot') == award.lotID]:
            request.errors.add('body', 'data', 'Can\'t update contract while cancellation for corresponding lot exists')
            request.errors.status = 403
            raise error_handler(request.errors)
...
def validate_contract_document_operation_not_in_allowed_contract_status(request):
    ...
    if contract.status not in ['pending', 'active']:
        request.errors.add('body', 'data', 'Can\'t {} document in current contract status'.format(OPERATIONS.get(request.method)))
        request.errors.status = 403
        raise error_handler(request.errors)
...
def validate_cancellation_in_termainated_status(request):
    ...
    if tender.status in ['complete', 'cancelled', 'unsuccessful']:
        request.errors.add('body', 'data', 'Can\'t {} cancellation in current ({}) tender status'.format(OPERATIONS.get(request.method), tender.status))
        request.errors.status = 403
        raise error_handler(request.errors)
