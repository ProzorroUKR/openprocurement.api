...
def create_tender_bid_invalid(self):
    ...
    response = self.app.post_json(request_path, {'data': {
        'selfEligible': True, 'selfQualified': True, 'tenderers': self.test_bids_data[0]['tenderers'],
        'lotValues': [{'value': {'yearlyPaymentsPercentage': 0.8, 'contractDuration': {'years': 12},
        'annualCostsReduction': [100]*21, 'currency': 'USD'}, 'relatedLot': self.initial_lots[0]['id']}]}},
         status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
...
def patch_tender_bid(self):
    ...
    self.assertEqual(response.json['data']['lotValues'][0]['value']['amountPerformance'], self.expected_bid_amountPerformance)
    ...
    response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(self.tender_id, bid['id'], bid_token),
        {'data': { 'lotValues': [{'value': {'currency': 'USD'}}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(self.tender_id, bid['id'], bid_token),
        {'data': { 'lotValues': [{'value': {'valueAddedTaxIncluded': False}}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    self.assertNotEqual(response.json['data']['lotValues'][0]['value']['amountPerformance'], self.expected_bid_amountPerformance)
