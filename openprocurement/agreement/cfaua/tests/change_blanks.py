...
def no_items_agreement_change(self):
    ...
    response = self.app.patch_json('/agreements/{}/credentials?acc_token={}'.format(agreement['id'], tender_token),
                                   {'data': ''})
...
def get_change(self):
    ...
    self.assertEqual(set(response.json['data']['changes'][0].keys()),
                     set(['id', 'date', 'status', 'rationaleTypes', 'rationale',
                          'rationale_ru', 'rationale_en', 'agreementNumber']))
...
def create_change_invalid(self):
    response = self.app.post('/agreements/{}/changes?acc_token={}'.format(self.agreement['id'], self.agreement_token),
                                  'data', status=415)
    ...
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token), {'data': {}}, status=422)
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token), {'data': {'rationale_ua': ""}}, status=422)
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "rationale_ua", "description": "Rogue field"}
    ])
    self.app.authorization = None
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token), {'data': {'rationale_ua': "aaa"}}, status=403)
...
def create_change(self):
    ...
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token), {'data':
            {'rationale': u'трататата', 'rationaleTypes': ['non-existing-rationale']}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "rationaleTypes", "description":
            [["Value must be one of ['volumeCuts', 'itemPriceVariation', 'qualityImprovement', 'thirdParty',"
              " 'durationExtension', 'priceReduction', 'taxRate', 'fiscalYearExtension']."]]}
    ])
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token),
...
def patch_change(self):
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'date': now}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.body, 'null')

    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'rationale_ru': 'шота на руськом'}})
    self.assertEqual(response.status, '200 OK')
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'rationaleTypes': []}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.json['errors'], [
    ...
    self.app.authorization = None
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'rationale_en': 'la-la-la'}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')

    self.app.authorization = ('Basic', ('broker', ''))
    response = self.app.patch_json('/agreements/{}/changes/{}'.format(self.agreement['id'], change['id']),
...
def change_date_signed(self):
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'dateSigned': "12-14-11"}}, status=422)
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'dateSigned': valid_date1}})
    self.assertEqual(response.status, '200 OK')
    ...
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "data", "description": "Can't update agreement change in current (active) status"}
    ])
    ...
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "dateSigned", "description": [u"Agreement signature date can't be in the future"]}
    ])
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change2['id'], self.agreement_token), {'data': {'dateSigned': date}})
    self.assertEqual(response.status, '200 OK')
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change2['id'], self.agreement_token), {'data': {'dateSigned': valid_date2}})
    self.assertEqual(response.status, '200 OK')
    ...
    date = get_now().isoformat()
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change3['id'], self.agreement_token), {'data': {'dateSigned': date}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.json['data']['dateSigned'], date)
...
def date_signed_on_change_creation(self):
    ...
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token),
              {'data': {'rationale': u'причина зміни укр', 'rationale_en': u'change cause en',
                        ...
                        'rationaleTypes': ['priceReduction'], 'agreementNumber': u'№ 146'}}, status=422)
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "dateSigned", "description": [u"Agreement signature date can't be in the future"]}
    ])
    date = get_now().isoformat()
    response = self.app.post_json('/agreements/{}/changes?acc_token={}'.format(
        self.agreement['id'], self.agreement_token),
              {'data': {'rationale': u'причина зміни укр', 'rationale_en': u'change cause en',
                        'dateSigned': date,
                        'rationaleTypes': ['priceReduction'], 'agreementNumber': u'№ 146'}})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    change = response.json['data']
...
def change_date_signed_very_old_agreements_data(self):
    ...
    agreement = self.db.get(self.agreement['id'])
    agreement['dateSigned'] = None
    self.db.save(agreement)
    ...
    response = self.app.patch_json('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], change['id'], self.agreement_token), {'data': {'status': 'active'}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'], [
        {"location": "body", "name": "data", "description":
            "Can't update agreement change status. 'dateSigned' is required."}
    ...
    response = self.app.get('/agreements/{}/changes/{}?acc_token={}'.format(
        self.agreement['id'], last_change['id'], self.agreement_token))
    self.assertEqual(response.status, '200 OK')
    self.assertNotIn('dateSigned', response.json['data'])
